...
mirna_counts_5prime = mirna_counts_5prime.drop('median', axis=1)
...
for key in distrib_dict.keys():
    ...
    extracted_mirna = dcona_runs.extract_samples(mirna_counts_5prime, descr.index)
    res = deseq(meta=descr, counts=extracted_mirna, formula="~Group", ref="Low_expr", exp="High_expr")
    res.to_csv(OUTPUT_PATH/'experiments'/key/'DESeq2_all_mirnas.tsv', sep='\t')